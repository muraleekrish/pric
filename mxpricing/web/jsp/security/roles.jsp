...
<jsp:useBean id="frm" class="com.savant.pricing.presentation.security.RoleListForm" />
	...
	try
	{
		...
		{  			
  		...
  		{  		   	
			...
			{
				System.out.println("frm.getTxtRoleName()"+frm.getTxtRoleName());
  		   		System.out.println("frm.getSelectUserType()"+frm.getSelectRoleName());  		   		
  		   	}
  		   	maxItems = Integer.parseInt(frm.getMaxItems());  		  	
  		  	Vector filter = new Vector();
			if (!frm.getTxtRoleName().trim().equalsIgnoreCase(""))
			{
		  		filter = new Vector();
		  		filter = objFilterHandler.setFormDetails("groupName",frm.getTxtRoleName(),frm.getSelectRoleName(),filter);
		  		fil = new Filter[filter.size()];
		 		filter.copyInto(fil);
		  		roleNamefilter = fil[0];
			}				
	  	}	
		maxItems = Integer.parseInt(frm.getMaxItems());
		pageCount = Integer.parseInt(frm.getPage());
		hmResult = rolesDAO.getAllRoles(roleNamefilter,"groupName",order,((pageCount-1)*maxItems),maxItems);
		totalRecords = Integer.parseInt(String.valueOf(hmResult.get("TotalRecordCount")));
		listRoles = (List)hmResult.get("Records");		
		...
		pageContext.setAttribute("listRoles",listRoles);
	}
	catch(Exception e)	
	{
		e.printStackTrace();
...
function clearSearch()
{
	try{
		temp=document.forms[0];
		temp.txtRoleName.value="";
		temp.selectRoleName.selectedIndex =0;
...
function editRole()
{
	try{
		...
		var obj = document.getElementById('roleLst');
		...
		if(count>0)
		{			
			temp.action='<%=request.getContextPath()%>/RoleEdit.do?roleid='+subQuery+'&formActoin=edit';		
			temp.formAction.value="edit";
			temp.submit();
		}
		else
		{
			alert("Please select a Role to Modify");
		}
	}catch(err)
	{
		alert(err.description);
...
function deleteRole()
{
		...
		for(var i=1;i<obj.children[0].children.length;i++)
		{
			...
			if(i == 1)
			{
				...
				{
					subQuery += obj.children[0].children[i].children[0].children[0].value+",";
					roleName = obj.children[0].children[i].children[1].innerHTML;
					count++;
				}
			}
			else
		...
		if(count>0)
		{
			...
			{	
				temp.action='<%=request.getContextPath()%>/RoleList.do?roleid='+subQuery+'&roleName='+roleName+'&formActoin=delete';
...
function loadDefault()
{
	...
	temp.txtRoleName.value = temp.txtRoleName.value;
}
...
{
	temp=document.forms[0];
	temp.action='<%=request.getContextPath()%>/jsp/security/roleview.jsp?id='+id;
	temp.submit();
}

</script>
<body onload='loadDefault();'> 
<html:form action="RoleList" focus="txtRoleName">
...
<table width="100%" height='100%' border="0" cellpadding="0" cellspacing="0"> 
  <tr> 
	...
	<table width="100%"  border="0" cellspacing="0" cellpadding="0"> 
      ...
      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="250" class="page_title">Roles </td>
          <td class="page_title"><table  border="0" align="right" cellpadding="0" cellspacing="0" class="topnav">
            <tr>
              <td><a href="<%=request.getContextPath()%>/jsp/security/roleadd.jsp">Add</a> | <a href="#" onclick="editRole()">Modify</a> | <a href="#" onclick="deleteRole()">Delete</a></td>
            </tr>
          </table></td>
        </tr>
      </table>
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="65" class="search">Role</td>
            <td width="1" class="search">:</td>
            <td class="search"><html:text maxlength="30" size="30" styleClass="textbox" property="txtRoleName" onkeypress="return checkEnter(event)"/>            
                <html:select property="selectRoleName" onchange='txtRoleName.focus();'>
              ...
              <html:button property="Button1" value="Go!" styleClass="button_sub_internal" onclick="search()" />
			  <html:button property="Button2" value="Clear" styleClass="button_sub_internal" onclick="clearSearch()"/>           
			 </td>
          </tr>
        </table>
       ...
       <div style="overflow:auto;height:expression((document.body.clientHeight - <%=browserHt%>) + 'px')" id="divList"> 
        <table width="100%"  border="0" cellspacing="0" cellpadding="0" id="roleLst">
          <tr class='staticHeader'>
            <td width="20" class="cmbheader">&nbsp;</td>
            <%if(order)
            {%>
            <td width="175" class="tblheader" id='img_sort' onClick="callSort();" align='center' style="cursor:hand" title='Sort by Role in Ascending'>Role <img src="<%=request.getContextPath()%>/images/sort.gif" width="7" height="8"></td>
            <%}else{%>
            <td width="175" class="tblheader" id='img_sort' onClick="callSort();" align='center' style="cursor:hand" title='Sort by Role in Decending'>Role <img src="<%=request.getContextPath()%>/images/sort_up.gif" width="7" height="8"></td>
            <%}%>
            <td class="tblheader" align='center'>Description</td>
          </tr>
          <% 
          if( listRoles.size() > 0 )
          {
          %>
		    <logic:iterate id="roles"  name="listRoles">
			...
			<td class="tbldata_chk" height='30'><input type="radio" name="checkbox" value="<bean:write name="roles" property="groupId"/>"></td>
			<td  class="tbldata" onclick="callView('<bean:write name="roles" property="groupId"/>')"><bean:write name="roles" property="groupName"/></td>
			<td  class="tbldata" onclick="callView('<bean:write name="roles" property="groupId"/>')"><bean:write name="roles" property="description"/>&nbsp;</td>
			</tr>
		    </logic:iterate>
		  <%
		  }
