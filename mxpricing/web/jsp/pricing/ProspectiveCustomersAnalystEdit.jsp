...
<%
	...
	else if((String)session.getAttribute("customerId")!=null && (!session.getAttribute("customerId").equals("")))
	{
		frm.setProspectiveCustId((String)session.getAttribute("customerId"));
	}
	else if(session.getAttribute("prsCustId")!= null)
	{
	frm.setProspectiveCustId((String)session.getAttribute("prsCustId"));
	}
	else
	{
		if(request.getParameter("prospectiveCustId")!= null && !request.getParameter("prospectiveCustId").equals("") )
		{
			frm.setProspectiveCustId(request.getParameter("prospectiveCustId"));
		}
		else
		{
			frm.setProspectiveCustId((String)request.getAttribute("prospectiveCustId"));
		}
  }

	session.setAttribute("prsCustId",frm.getProspectiveCustId());
...
function importPic()
{
	...
	temp.action='<%=request.getContextPath()%>/FileUpload.do?prsCustId=<%=frm.getProspectiveCustId()%>';
...
function customerEdit()
{
	try{
	var temp = document.forms[0];
	temp.action='<%=request.getContextPath()%>/ProspectiveCustomerRepEdit.do?prsCustId='+<%=frm.getProspectiveCustId()%>+'&User=Analyst';
...
function callList()
{
   try{
	var temp = document.forms[0];
	temp.action='<%=request.getContextPath()%>/jsp/pricing/ProspectiveCustomersAnalyst.jsp';
...
function actioncontrol(formaction)
{
	...
	temp.prospectiveCustId.value = <%=frm.getProspectiveCustId()%>
	temp.submit();
}
</script>
</head>
<body > 
<html:form action="ProspectiveCustomerAnalystEdit">  
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td width="82" class="subtab_off" ><a href="#" onclick="importPic()" >Import&nbsp;CUD</a></td>
            <td width="82" class="subtab_off" ><a href="<%=request.getContextPath()%>/PreferenceAnalyst.do?prsCustId=<%=frm.getProspectiveCustId()%>" >&nbsp;Select ESIIDs</a></td>
		<%if(!objProspectiveCustomerVO.isMmCust())
		{
		%>
			 <td width="82" class="subtab_off" ><a href="<%=request.getContextPath()%>/Ecomp.do?prsCustId=<%=frm.getProspectiveCustId()%>" >&nbsp;Select&nbsp;Components</a></td>
		 <%}%>

           
            <td width="80" class="subtab_off"><a href="<%=request.getContextPath()%>/DealLeverAnalyst.do?prsCustId=<%=frm.getProspectiveCustId()%>">Deal&nbsp;Adjustment</a></td>
            <td>&nbsp;</td>
          </tr>
        </table>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr id="tblPremium">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
                  ...
                  <td width="1" class="fieldtitle">:</td>
                  <td  class="fieldata"><bean:write name="objProspectiveCustomerVO" property="customerStatus.customerStatus" ignore="true"/></td>	
				...
				<!--   <td width="119" class="fieldtitle">Consultant</td>
                  ...
                  %> 
                  <td  class="fieldata"><%=strlocType%></td>		
                </tr>
				<tr>
                  ...
                  <td width="1" class="fieldtitle">:</td>
				  <td width="170" class="fieldata"><bean:write name="objProspectiveCustomerVO" property="cdrStatus.cdrState" ignore="true"/></td>
                </tr>
			</table>
			</fieldset>
			<br>
			...
			<fieldset id="comments" style='align:center; margin: -5px 10px 0px 10px'>
              <legend>Comments-History</legend>
				<div style="overflow:auto;height:125" id="divList"> 
				 <table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<table width="100%"  border="0" cellspacing="0" cellpadding="0"> 
					<tr> 
						<td class="fieldata">
							...
							<html:button  property="Next" styleClass="button" value="Next" onclick="importPic()"></html:button>
							<html:button  property="Cancel" styleClass="button" value="Cancel" onclick="callList();"></html:button>
						</td>
					</tr> 
				</table>
            </td>
