...
<%@ page import="com.savant.pricing.dao.ServiceVoltageDAO"%>
<%@ page import="com.savant.pricing.common.FilterHandler"%>
<%@ page import="com.savant.pricing.valueobjects.ServiceVoltageVO" %>
 
<jsp:useBean id="frm" class="com.savant.pricing.presentation.global.tdsp.ServiceVoltageListForm" />
...
<%
	...
	try
	{
		...
		Filter fil[]=null;
		Filter serviceVoltagefilter  = null;
		if(frm.getFormActions().equals(""))
		{
  		...
  		{ 	
			...
			if (!frm.getTxtServiceVoltage().trim().equalsIgnoreCase(""))
			{
		  		filter = new Vector();
		  		filter = objFilterHandler.setFormDetails("serviceVoltageType",frm.getTxtServiceVoltage(),frm.getSearchServiceVoltage(),filter);
		  		fil = new Filter[filter.size()];
		 		filter.copyInto(fil);
		  		serviceVoltagefilter = fil[0];
			}
		}
		...
		hmResult = objServiceVoltageDAO.getAllServiceVoltage(serviceVoltagefilter,"serviceVoltageType",order,((pageCount-1)*maxItems),maxItems);
		totalRecords = Integer.parseInt(String.valueOf(hmResult.get("TotalRecordCount")));
		if(hmResult!=null)
			 lstServiceVoltage =(List)hmResult.get("Records");
		...
		pageContext.setAttribute("lstServiceVoltage",lstServiceVoltage);
...
{	
	...
	temp.txtServiceVoltage.value = temp.txtServiceVoltage.value;
}
function search()
{
	temp=document.forms[0];
...
{
	temp = document.forms[0];
	temp.txtServiceVoltage.value = '';
	temp.searchServiceVoltage.value = '0';
}
...
function changeStatus()
{
	try
	{
		...
		queryString = "";
		temp    = document.forms[0];
		var obj = document.getElementById( 'serviceLst' );
		flag = 0;
		...
		if( flag  )
		{
			...
			temp.serviceId.value   = queryString;
			temp.submit();
		}
		else
		{
...
<html:form action="ServiceVoltageType" focus='txtServiceVoltage'>
  ...
  <tr> 
   <td  valign="top"> <jsp:include page="/jsp/menu.jsp"/>
		<table width = '100%' border = '0' cellspacing = '0' cellpadding = '0'> 
   		...
   		<table width="100%"  border="0" cellspacing="0" cellpadding="0">
        	<tr>
          		<td class="page_title">Service Voltage</td>
          		<td class = 'page_title'>
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td width="120" class="search">Service Voltage Type</td>
            <td width="12" class="search">:</td>
            <td class="search"><html:text property="txtServiceVoltage" styleClass="textbox" size="15" onkeypress="return checkEnter(event)"/>
               <html:select property="searchServiceVoltage" onchange='document.forms[0].btnSubmit.focus();'>
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr class='staticHeader'>
            ...
            <%if(order)
            {%>
            <td width="186" class="tblheader" id='img_sort' onClick="callSort();" style="cursor:hand" title='Sort by Service Voltage Type in Ascending'>Service Voltage Type <img src="<%=request.getContextPath()%>/images/sort.gif" width="7" height="8"></td>
            <%}else{%>
            <td width="186" class="tblheader" id='img_sort' onClick="callSort();" style="cursor:hand" title='Sort by Service Voltage Type in Decending'>Service Voltage Type <img src="<%=request.getContextPath()%>/images/sort_up.gif" width="7" height="8"></td>
            <%}%>
            <td class="tblheader">Description</td>
            <td width = '100' class = 'tblheader'>Is Valid</td>
          </tr>
        </table>
        <table width="100%"  border="0" cellspacing="0" cellpadding="0" id='serviceLst'>
        <% 
          if( lstServiceVoltage.size() > 0 )
          {
        %>
		<logic:iterate id="ServiceVoltage"  name="lstServiceVoltage" indexId = 'i'>
		  <%
				int val           = i.intValue();
				boolean runStatus = false;
				runStatus = ((ServiceVoltageVO)lstServiceVoltage.get(val)).isValid();
		  %>
          <tr>
            <td width="19" class="tbldata">
			<input type="radio" name="checkbox" value='<bean:write name="ServiceVoltage" property="serviceVoltageIdentifier"/>'>
			</td>
             <td width="186" class="tbldata"><bean:write name="ServiceVoltage" property="serviceVoltageType" /></td>
           <td class="tbldata"><bean:write name="ServiceVoltage" property="description" />&nbsp;</td> 
