...
<%@ page import="com.savant.pricing.valueobjects.ProductsVO" %>
<%@ page import="com.savant.pricing.common.FilterHandler"%>
<jsp:useBean id="frm" class="com.savant.pricing.presentation.pconfiguration.ProductsListForm" />
	...
	try
	{
		...
		if(frm.getFormActions().equals(""))
		{
  		...
  		{ 	
			...
			if (!frm.getTxtProductName().trim().equalsIgnoreCase(""))
			{
		  		filter = new Vector();
		  		filter = objFilterHandler.setFormDetails("productName",frm.getTxtProductName(),frm.getSearchProductName(),filter);
		  		fil = new Filter[filter.size()];
		 		filter.copyInto(fil);
		  		productNamefilter = fil[0];
			}
		}
		...
		hmResult = objProductsDAO.getAllProducts(productNamefilter,"productName",order,((pageCount-1)*maxItems),maxItems,false);
		totalRecords = Integer.parseInt(String.valueOf(hmResult.get("TotalRecordCount")));
		if(hmResult!=null)
			 lstProducts =(List)hmResult.get("Records");
		totalRecords = Integer.parseInt(hmResult.get("TotalRecordCount").toString());
		pageContext.setAttribute("lstProducts",lstProducts);
...
{	
	...
	temp.txtProductName.value = temp.txtProductName.value;
}
function fnClear()
{
	temp = document.forms[0];
	temp.txtProductName.value = '';
	temp.searchProductName.value = '0';
}
...
function changeStatus()
{
	try
	{
		...
		queryString = "";
		temp    = document.forms[0];
		var obj = document.getElementById( 'productLst' );
		flag = 0;
		...
		if( flag  )
		{
			...
			temp.prductsId.value   = queryString;
			temp.submit();
		}
		else
		{
			alert( 'Please select a Product List' );
		}
	}
	catch( err )
	{
...
<html:form action="productsList" focus='txtProductName'>
<html:hidden property="formActions"/>
<html:hidden property="prductsId"/>
  ...
  <tr> 
   <td  valign="top"> <jsp:include page="/jsp/menu.jsp"/>
		<table width = '100%' border = '0' cellspacing = '0' cellpadding = '0'> 
   		...
   		<table width="100%"  border="0" cellspacing="0" cellpadding="0">
        	<tr>
          		<td width = '250' class="page_title">Products</td>
          		<td class = 'page_title'>
        ...
        <table width="100%"  border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td width="100" class="search">Product Name</td>
            <td width="12" class="search">:</td>
            <td class="search"><html:text property="txtProductName" styleClass="textbox" size="15" onkeypress="return checkEnter(event)"/>
               <html:select property="searchProductName">
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr class='staticHeader'>
            ...
            <%if(order)
            {%>
            <td width="186" class="tblheader" id='img_sort' onClick="callSort();" style="cursor:hand" title='Sort by Product Name in Ascending'>Product Name <img src="images/sort.gif" width="7" height="8"></td>
            <%}else{%>
            <td width="186" class="tblheader" id='img_sort' onClick="callSort();" style="cursor:hand" title='Sort by Product Name in Decending'>Product Name <img src="images/sort_up.gif" width="7" height="8"></td>
            <%}%>
            <td class="tblheader">Description</td>
            <td width = '100' class = 'tblheader'>Is Valid</td>
          </tr>
        </table>
        <table width="100%"  border="0" cellspacing="0" cellpadding="0" id='productLst'>
        <% 
          if( lstProducts.size() > 0 )
          {
        %>
		<logic:iterate id="products"  name="lstProducts" indexId = 'i'>
		  <%
				int val           = i.intValue();
				boolean runStatus = false;
				runStatus = ((ProductsVO)lstProducts.get(val)).isValid();
		  %>
          <tr>
            <td width="19" class="tbldata">
			<input type="radio" name ='checkValue' value='<bean:write name="products" property="productIdentifier"/>' >
			</td>
             <td width="186" class="tbldata"><bean:write name="products" property="productName" /></td>
           <td class="tbldata"><bean:write name="products" property="description" />&nbsp;</td> 
