/*
 * The Mira DHTML Calendar
 *
 * Copyright (c) 2004 by Mira, Inc.
 * http://www.Mira.com
...
// $Id: calendar.js,v 1.1 2005/06/06 10:11:54 ahariharan Exp $
 ...
 *      cal.destroy();
 *   }
 *   var cal = new Mira.Calendar(1, new Date(), selFunc, closeFunc);
 * \endcode
 *
 ...
 * calendar using Mira.Calendar.create().
 *
 ...
 */
Mira.Calendar = function (firstDayOfWeek, dateStr, onSelected, onClose) {
	// member variables
	this.activeDiv = null;
	...
	this.minMonth = 0;
	this.maxMonth = 11;
	this.dateFormat = Mira.Calendar.i18n("DEF_DATE_FORMAT");
	this.ttDateFormat = Mira.Calendar.i18n("TT_DATE_FORMAT");
	this.historyDateFormat = "%B %d, %Y";
	this.isPopup = true;
	this.weekNumbers = true;
	this.noGrab = false;
	if (Mira.Calendar.prefs.fdow || (Mira.Calendar.prefs.fdow == 0)) {
		this.firstDayOfWeek = parseInt(Mira.Calendar.prefs.fdow, 10);
	}
	else {
		var fd = 0;
		if (typeof firstDayOfWeek == "number") {
			fd = firstDayOfWeek;
		} else if (typeof Mira.Calendar._FD == 'number') {
			fd = Mira.Calendar._FD;
		}
 ...
 */
Mira.Calendar._initSDN = function() {
	if (typeof Mira.Calendar._TT._SDN == "undefined") {
		// table of short day names
		if (typeof Mira.Calendar._TT._SDN_len == "undefined")
			Mira.Calendar._TT._SDN_len = 3;
		var ar = [];
		for (var i = 8; i > 0;) {
			ar[--i] = Mira.Calendar._TT._DN[i].substr(0, Mira.Calendar._TT._SDN_len);
		}
		Mira.Calendar._TT._SDN = ar;
		// table of short month names
		if (typeof Mira.Calendar._TT._SMN_len == "undefined")
			Mira.Calendar._TT._SMN_len = 3;
		ar = [];
		for (var i = 12; i > 0;) {
			ar[--i] = Mira.Calendar._TT._MN[i].substr(0, Mira.Calendar._TT._SMN_len);
		}
		Mira.Calendar._TT._SMN = ar;
	}
};
 ...
 */
Mira.Calendar.i18n = function(str, type) {
	var tr = '';
	if (!type) {
		// normal _TT request
		if (Mira.Calendar._TT)
			tr = Mira.Calendar._TT[str];
		if (!tr && Mira.Calendar._TT_en)
			tr = Mira.Calendar._TT_en[str];
	} else switch(type) {
	    case "dn"  : tr = Mira.Calendar._TT._DN[str];  break;
	    case "sdn" : tr = Mira.Calendar._TT._SDN[str]; break;
	    case "mn"  : tr = Mira.Calendar._TT._MN[str];  break;
	    case "smn" : tr = Mira.Calendar._TT._SMN[str]; break;
	}
	if (!tr) tr = "" + str;
	return tr;
};
...
Mira.Calendar._C = null;

/// preferences
Mira.Calendar.prefs = {
	fdow     : null,	/**< when NULL we will use the options passed at Mira.Calendar.setup */
 ...
 */
Mira.Calendar.savePrefs = function() {
	// FIXME: should we make the domain, path and expiration time configurable?
	// I guess these defaults are right though..
	Mira.Utils.writeCookie("DYN_CAL", Mira.Utils.makePref(this.prefs), null, '/', 30);
};

/**
 * Loads the preference cookie and merges saved prefs to Mira.Calendar.prefs.
 */
Mira.Calendar.loadPrefs = function() {
	var txt = Mira.Utils.getCookie("DYN_CAL"), tmp;
	if (txt) {
		tmp = Mira.Utils.loadPref(txt);
		if (tmp)
			Mira.Utils.mergeObjects(this.prefs, tmp);
	}
	// FIXME: DEBUG!
 ...
 */
Mira.Calendar._add_evs = function(el) {
	var C = Mira.Calendar;
	Mira.Utils.addEvent(el, "mouseover", C.dayMouseOver);
	Mira.Utils.addEvent(el, "mousedown", C.dayMouseDown);
	Mira.Utils.addEvent(el, "mouseout", C.dayMouseOut);
	if (Mira.is_ie)
		Mira.Utils.addEvent(el, "dblclick", C.dayMouseDblClick);
};

/**
 * \internal This function undoes what Mira.Calendar._add_evs did, therefore
 * unregisters the event handlers.
 *
 * @param el [HTMLElement] reference to your element.
 */
Mira.Calendar._del_evs = function(el) {
	var C = this;
	Mira.Utils.removeEvent(el, "mouseover", C.dayMouseOver);
	Mira.Utils.removeEvent(el, "mousedown", C.dayMouseDown);
	Mira.Utils.removeEvent(el, "mouseout", C.dayMouseOut);
	if (Mira.is_ie)
		Mira.Utils.removeEvent(el, "dblclick", C.dayMouseDblClick);
};
 ...
 */
Mira.Calendar.findMonth = function(el) {
...
Mira.Calendar.findHist = function(el) {
...
Mira.Calendar.findYear = function(el) {
 ...
 */
Mira.Calendar.showMonthsCombo = function () {
	var cal = Mira.Calendar._C;
	if (!cal) {
		return false;
	}
	...
	for (var i = mc.firstChild; i; i = i.nextSibling) {
		var m = i.month;
		Mira.Utils.removeClass(i, "hilite");
		Mira.Utils.removeClass(i, "active");
		Mira.Utils.removeClass(i, "disabled");
		i.disabled = false;
		if ((min && m < cal.minMonth) ||
		    (max && m > cal.maxMonth)) {
			Mira.Utils.addClass(i, "disabled");
			i.disabled = true;
		}
		if (m == MM)
			Mira.Utils.addClass(cal.activeMonth = i, "active");
	}
	var s = mc.style;
	s.display = "block";
	if (cd.navtype < 0)
 ...
 * active calendar.
 */
Mira.Calendar.showHistoryCombo = function() {
	var cal = Mira.Calendar._C, a, h, i, cd, hc, s, tmp, div;
	if (!cal)
		return false;
	hc = cal.histCombo;
	while (hc.firstChild)
		hc.removeChild(hc.lastChild);
	if (Mira.Calendar.prefs.history) {
		a = Mira.Calendar.prefs.history.split(/,/);
		i = 0;
		while (tmp = a[i++]) {
			tmp = tmp.split(/\//);
			h = Mira.Utils.createElement("div");
			h.className = Mira.is_ie ? "label-IEfix" : "label";
 ...
 * otherwise.
 */
Mira.Calendar.showYearsCombo = function (fwd) {
	var cal = Mira.Calendar._C;
	if (!cal) {
		return false;
	}
	...
	if (cal.hilitedYear) {
		Mira.Utils.removeClass(cal.hilitedYear, "hilite");
	}
	if (cal.activeYear) {
		Mira.Utils.removeClass(cal.activeYear, "active");
	}
	cal.activeYear = null;
 ...
 *
 * Also, the Mira.Calendar.cellClick() function is called, which further
 ...
 * @return false
 */
Mira.Calendar.tableMouseUp = function(ev) {
	var cal = Mira.Calendar._C;
	if (!cal) {
		return false;
	}
	...
	var target = Mira.Utils.getTargetElement(ev);
	ev || (ev = window.event);
	Mira.Utils.removeClass(el, "active");
	if (target == el || target.parentNode == el) {
		Mira.Calendar.cellClick(el, ev);
	}
	var mon = Mira.Calendar.findMonth(target);
	var date = null;
	if (mon) {
		if (!mon.disabled) {
		...
		var year = Mira.Calendar.findYear(target);
		if (year) {
			date = new Date(cal.date);
			...
			var hist = Mira.Calendar.findHist(target);
	...
	Mira.Utils.removeEvent(window.document, "mouseup", Mira.Calendar.tableMouseUp);
	Mira.Utils.removeEvent(window.document, "mouseover", Mira.Calendar.tableMouseOver);
	Mira.Utils.removeEvent(window.document, "mousemove", Mira.Calendar.tableMouseOver);
	cal._hideCombos();
	Mira.Calendar._C = null;
	return Mira.Utils.stopEvent(ev);
};
 ...
 * @return
 */
Mira.Calendar.tableMouseOver = function (ev) {
	var cal = Mira.Calendar._C;
	if (!cal) {
		return;
	}
	...
	var target = Mira.Utils.getTargetElement(ev);
	if (target == el || target.parentNode == el) {
		Mira.Utils.addClass(el, "hilite active");
		Mira.Utils.addClass(el.parentNode, "rowhilite");
	} else {
		if (typeof el.navtype == "undefined" ||
		    (el.navtype != 50 && ((el.navtype == 0 && !cal.histCombo) || Math.abs(el.navtype) > 2)))
			Mira.Utils.removeClass(el, "active");
		Mira.Utils.removeClass(el, "hilite");
		Mira.Utils.removeClass(el.parentNode, "rowhilite");
	}
	ev || (ev = window.event);
	if (el.navtype == 50 && target != el) {
		var pos = Mira.Utils.getAbsolutePos(el);
	...
	if (mon) {
		if (!mon.disabled) {
			if (mon.month != cal.date.getMonth()) {
				if (cal.hilitedMonth) {
					Mira.Utils.removeClass(cal.hilitedMonth, "hilite");
				}
				Mira.Utils.addClass(mon, "hilite");
		...
		if (year) {
			if (year.year != cal.date.getFullYear()) {
				...
				Mira.Utils.addClass(year, "hilite");
			...
			if (hist) {
				if (!hist.histDate.dateEqualsTo(cal.date)) {
					if (cal.hilitedHist) {
						Mira.Utils.removeClass(cal.hilitedHist, "hilite");
					}
					Mira.Utils.addClass(hist, "hilite");
 ...
 * @return false
 */
Mira.Calendar.tableMouseDown = function (ev) {
	if (Mira.Utils.getTargetElement(ev) == Mira.Utils.getElement(ev)) {
		return Mira.Utils.stopEvent(ev);
	}
};
 ...
 * @return false
 */
Mira.Calendar.calDragIt = function (ev) {
	ev || (ev = window.event);
	var cal = Mira.Calendar._C;
	if (!(cal && cal.dragging)) {
		return false;
	...
	st.left = L + "px";
	st.top = T + "px";
	Mira.Utils.setupWCH(cal.WCH, L, T);
	return Mira.Utils.stopEvent(ev);
};
 ...
 * Mira.Calendar.hideShowCovered() which repaints as appropriate any
 ...
 * @return false
 */
Mira.Calendar.calDragEnd = function (ev) {
	var cal = Mira.Calendar._C;
	if (!cal) {
		return false;
	}
	cal.dragging = false;
	Mira.Utils.removeEvent(window.document, "mousemove", Mira.Calendar.calDragIt);
	Mira.Utils.removeEvent(window.document, "mouseover", Mira.Calendar.calDragIt);
	Mira.Utils.removeEvent(window.document, "mouseup", Mira.Calendar.calDragEnd);
	Mira.Calendar.tableMouseUp(ev);
	cal.hideShowCovered();
};
//@}
 ...
 * Mira.Calendar.tableMouseOver() event handler on the document.  If the
 ...
 * @return false
 */
Mira.Calendar.dayMouseDown = function(ev) {
	var el = Mira.Utils.getElement(ev);
	if (el.disabled) {
		return false;
	}
	var cal = el.calendar;
	cal.activeDiv = el;
	Mira.Calendar._C = cal;
	if (el.navtype != 300) {
		if (el.navtype == 50) {
			el._current = el.firstChild.data;
			Mira.Utils.addEvent(window.document, "mousemove", Mira.Calendar.tableMouseOver);
		} else
			Mira.Utils.addEvent(window.document, Mira.is_ie5 ? "mousemove" : "mouseover", Mira.Calendar.tableMouseOver);
		Mira.Utils.addClass(el, "hilite active");
		Mira.Utils.addEvent(window.document, "mouseup", Mira.Calendar.tableMouseUp);
	} else if (cal.isPopup) {
		cal._dragStart(ev);
	}
	if (el.navtype == -1 || el.navtype == 1) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout("Mira.Calendar.showMonthsCombo()", 250);
	} else if (el.navtype == -2 || el.navtype == 2) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout((el.navtype > 0) ? "Mira.Calendar.showYearsCombo(true)" : "Mira.Calendar.showYearsCombo(false)", 250);
	} else if (el.navtype == 0 && Mira.Calendar.prefs.history) {
		if (cal.timeout) clearTimeout(cal.timeout);
		cal.timeout = setTimeout("Mira.Calendar.showHistoryCombo()", 250);
	} else {
		cal.timeout = null;
	}
	return Mira.Utils.stopEvent(ev);
 ...
 */
Mira.Calendar.dayMouseDblClick = function(ev) {
	Mira.Calendar.cellClick(Mira.Utils.getElement(ev), ev || window.event);
	if (Mira.is_ie)
 ...
 * @return false
 */
Mira.Calendar.dayMouseOver = function(ev) {
	var el = Mira.Utils.getElement(ev),
		caldate = el.caldate;
	if (caldate)
		caldate = new Date(caldate[0], caldate[1], caldate[2]);
	if (Mira.Utils.isRelated(el, ev) || Mira.Calendar._C || el.disabled) {
		return false;
	}
	if (el.ttip) {
 ...
 * @return false
 */
Mira.Calendar.dayMouseOut = function(ev) {
	var el = Mira.Utils.getElement(ev);
	if (Mira.Utils.isRelated(el, ev) || Mira.Calendar._C || el.disabled)
		return false;
	Mira.Utils.removeClass(el, "hilite");
	if (el.caldate)
		Mira.Utils.removeClass(el.parentNode, "rowhilite");
	if (el.calendar)
		el.calendar.showHint(Mira.Calendar.i18n("SEL_DATE"));
	return Mira.Utils.stopEvent(ev);
};
 ...
 * @param ev [Event] the event object
 */
Mira.Calendar.cellClick = function(el, ev) {
	var cal = el.calendar;
	var closing = false;
	var newdate = false;
	var date = null;
	if (typeof el.navtype == "undefined") {
		if (cal.currentDateEl) {
			Mira.Utils.removeClass(cal.currentDateEl, "selected");
			Mira.Utils.addClass(el, "selected");
		...
		switch (el.navtype) {
		    case 400:
			Mira.Utils.removeClass(el, "hilite");
			var text = Mira.Calendar.i18n("ABOUT");
			if (typeof text != "undefined") {
				text += cal.showsTime ? Mira.Calendar.i18n("ABOUT_TIME") : "";
			} else {
				// FIXME: this should be removed as soon as lang files get updated!
				text = "Help and about box text is not translated into this language.\n" +
					"If you know this language and you feel generous please update\n" +
					"the corresponding file in \"lang\" subdir to match calendar-en.js\n" +
					"and send it back to <support@Mira.com> to get it into the distribution  ;-)\n\n" +
					"Thank you!\n" +
					"http://www.Mira.com\n";
			}
			alert(text);
			...
			Mira.Calendar.prefs.fdow = cal.firstDayOfWeek;
			Mira.Calendar.savePrefs();
			if (cal.onFDOW)
 ...
 * Mira.Calendar.prototype._init().  Therefore, by separating these 2
 ...
 * @param _par
 */
Mira.Calendar.prototype.create = function (_par) {
	var parent = null;
	if (! _par) {
	...
	var table = Mira.Utils.createElement("table");
	this.table = table;
	table.cellSpacing = 0;
	table.cellPadding = 0;
	table.calendar = this;
	Mira.Utils.addEvent(table, "mousedown", Mira.Calendar.tableMouseDown);

	var div = Mira.Utils.createElement("div");
	this.element = div;
	...
	var thead = Mira.Utils.createElement("thead", table);
	var cell = null;
	var row = null;

	var cal = this;
	var hh = function (text, cs, navtype) {
		cell = Mira.Utils.createElement("td", row);
		cell.colSpan = cs;
		cell.className = "button";
		if (Math.abs(navtype) <= 2)
			cell.className += " nav";
		Mira.Calendar._add_evs(cell);
		cell.calendar = cal;
	...
	row = Mira.Utils.createElement("tr", thead);
	var title_length = 6;
	this.isPopup && --title_length;
	this.weekNumbers && ++title_length;

	hh("?", 1, 400).ttip = Mira.Calendar.i18n("INFO");
	this.title = hh("", title_length, 300);
	this.title.className = "title";
	if (this.isPopup) {
		this.title.ttip = Mira.Calendar.i18n("DRAG_TO_MOVE");
		this.title.style.cursor = "move";
		hh("&#x00d7;", 1, 200).ttip = Mira.Calendar.i18n("CLOSE");
	}
	...
	this._nav_py = hh("&#x00ab;", 1, -2);
	this._nav_py.ttip = Mira.Calendar.i18n("PREV_YEAR");

	this._nav_pm = hh("&#x2039;", 1, -1);
	this._nav_pm.ttip = Mira.Calendar.i18n("PREV_MONTH");

	this._nav_now = hh(Mira.Calendar.i18n("TODAY"), this.weekNumbers ? 4 : 3, 0);
	this._nav_now.ttip = Mira.Calendar.i18n("GO_TODAY");

	this._nav_nm = hh("&#x203a;", 1, 1);
	this._nav_nm.ttip = Mira.Calendar.i18n("NEXT_MONTH");

	this._nav_ny = hh("&#x00bb;", 1, 2);
	this._nav_ny.ttip = Mira.Calendar.i18n("NEXT_YEAR");
	...
	if (this.weekNumbers) {
		...
		cell.appendChild(window.document.createTextNode(Mira.Calendar.i18n("WK")));
		var cal_wk = Mira.Calendar.i18n("WK")
			if (cal_wk == null) {
	...
	var tbody = Mira.Utils.createElement("tbody", table);
	this.tbody = tbody;

	for (i = 6; i > 0; --i) {
		row = Mira.Utils.createElement("tr", tbody);
		if (this.weekNumbers) {
			cell = Mira.Utils.createElement("td", row);
	...
	var tfoot = Mira.Utils.createElement("tfoot", table);

	if (this.showsTime) {
		row = Mira.Utils.createElement("tr", tfoot);
		row.className = "time";

		cell = Mira.Utils.createElement("td", row);
		cell.className = "timetext";
		cell.colSpan = this.weekNumbers ? 2 : 1;
		cell.innerHTML = Mira.Calendar.i18n("TIME") || "&nbsp;";
		...
		(function(){
			function makeTimePart(className, init, range_start, range_end) {
				...
				if (range_end) {
					...
					table = Mira.Utils.createElement("table", cell);
					table.cellSpacing = table.cellPadding = 0;
					if (className == "hour")
						table.align = "right";
					table.className = "calendar-time-scroller";
					tbody = Mira.Utils.createElement("tbody", table);
					tr    = Mira.Utils.createElement("tr", tbody);
					tr2   = Mira.Utils.createElement("tr", tbody);
				} else
					tr = row;
				part = Mira.Utils.createElement("td", tr);
				part.className = className;
				part.appendChild(window.document.createTextNode(init));
				part.calendar = cal;
				part.ttip = Mira.Calendar.i18n("TIME_PART");
				part.navtype = 50;
				part._range = [];
				if (!range_end)
					...
					var up = Mira.Utils.createElement("td", tr);
					up.className = "up";
					up.navtype = 201;
					up.calendar = cal;
					up.timePart = part;
					if (Mira.is_khtml)
						up.innerHTML = "&nbsp;";
					Mira.Calendar._add_evs(up);

					var down = Mira.Utils.createElement("td", tr2);
					down.className = "down";
					down.navtype = 202;
					down.calendar = cal;
					down.timePart = part;
					if (Mira.is_khtml)
						down.innerHTML = "&nbsp;";
					Mira.Calendar._add_evs(down);
				}
				Mira.Calendar._add_evs(part);
				return part;
			};
			...
			if (t12) {
				...
				Mira.Utils.createElement("td", row).innerHTML = "&nbsp;";
	...
	cell = hh(Mira.Calendar.i18n("SEL_DATE"), this.weekNumbers ? 8 : 7, 300);
	cell.className = "ttip";
	if (this.isPopup) {
		cell.ttip = Mira.Calendar.i18n("DRAG_TO_MOVE");
		cell.style.cursor = "move";
	}
	this.tooltips = cell;

	div = this.monthsCombo = Mira.Utils.createElement("div", this.element);
	div.className = "combo";
	for (i = 0; i < 12; ++i) {
		var mn = Mira.Utils.createElement("div");
		mn.className = Mira.is_ie ? "label-IEfix" : "label";
		mn.month = i;
		mn.appendChild(window.document.createTextNode(Mira.Calendar.i18n(i, "smn")));
		div.appendChild(mn);
	}

	div = this.yearsCombo = Mira.Utils.createElement("div", this.element);
	div.className = "combo";
	for (i = 12; i > 0; --i) {
		var yr = Mira.Utils.createElement("div");
		yr.className = Mira.is_ie ? "label-IEfix" : "label";
		yr.appendChild(window.document.createTextNode(""));
		div.appendChild(yr);
	}

	div = this.histCombo = Mira.Utils.createElement("div", this.element);
 ...
 * @return false
 */
Mira.Calendar._keyEvent = function(ev) {
	if (!window.calendar) {
		return false;
	}
	(Mira.is_ie) && (ev = window.event);
	var cal = window.calendar;
	var act = (Mira.is_ie || ev.type == "keypress");
	var K = ev.keyCode;
	if (ev.ctrlKey) {
		switch (K) {
		    case 37: // KEY left
			act && Mira.Calendar.cellClick(cal._nav_pm);
			break;
		    case 38: // KEY up
			act && Mira.Calendar.cellClick(cal._nav_py);
			break;
		    case 39: // KEY right
			act && Mira.Calendar.cellClick(cal._nav_nm);
			break;
		    case 40: // KEY down
			act && Mira.Calendar.cellClick(cal._nav_ny);
			break;
		    default:
			return false;
		}
		...
		Mira.Calendar.cellClick(cal._nav_now);
		break;
	    case 27: // KEY esc
		...
		if (act) {
			...
			if (ne) {
				if (!ne.otherMonth) {
					Mira.Utils.removeClass(el, "selected");
					Mira.Utils.addClass(ne, "selected");
					cal.date.setDateOnly(new Date(ne.caldate[0], ne.caldate[1], ne.caldate[2]));
					cal.currentDateEl = ne;
				} else {
					if (!ne.disabled)
						Mira.Calendar.cellClick(ne);
					else if (prev)
						prevMonth();
					else
						nextMonth();
 ...
 * @return
 */
Mira.Calendar.prototype._init = function (firstDayOfWeek, date) {
	var
		today = new Date(),
		TD = today.getDate(),
	...
	var MN = Mira.Calendar.i18n(month, "smn");
	var ar_days = this.ar_days = [];
	var weekend = Mira.Calendar.i18n("WEEKEND");
		...
		for (; cell && (iday = date.getDate()); date.setDate(iday+1), cell = cell.nextSibling) {
			...
			if (!cell.disabled) {
				...
				if (dyear == TY && dmonth == TM && iday == TD) {
					...
					cell.ttip += Mira.Calendar.i18n("PART_TODAY");
				}
	...
	//this.title.firstChild.data = Mira.Calendar.i18n(month, "mn") + ", " + year;
	this.title.innerHTML = Mira.Calendar.i18n(month, "mn") + ", " + year;
	this.onSetTime();
 ...
 */
Mira.Calendar.prototype._initMultipleDates = function() {
	if (this.multiple) {
 ...
 */
Mira.Calendar.prototype._toggleMultipleDate = function(date) {
	if (this.multiple) {
		...
		if (cell) {
			...
			if (!d) {
				Mira.Utils.addClass(cell, "selected");
				this.multiple[ds] = date;
			} else {
				Mira.Utils.removeClass(cell, "selected");
 ...
 */
Mira.Calendar.prototype.setDateToolTipHandler = function (unaryFunction) {
 ...
 */
Mira.Calendar.prototype.setDate = function (date) {
	...
	if (!date.equalsTo(this.date)) {
		...
		if (year == this.minYear && m < this.minMonth)
			this.showHint("<div class='error'>" + Mira.Calendar.i18n("E_RANGE") + " »»»</div>");
 ...
 */
Mira.Calendar.prototype.showHint = function(text) {
 ...
 */
Mira.Calendar.prototype.reinit = function() {
	this._init(this.firstDayOfWeek, this.date);
};
 ...
 */
Mira.Calendar.prototype.refresh = function() {
 ...
 */
Mira.Calendar.prototype.setFirstDayOfWeek = function (firstDayOfWeek) {
 ...
 */
Mira.Calendar.prototype.setDateStatusHandler = Mira.Calendar.prototype.setDisabledHandler = function (unaryFunction) {
 ...
 */
Mira.Calendar.prototype.setRange = function (A, Z) {
	var m,
		a = Math.min(A, Z),
		z = Math.max(A, Z);
 ...
 * calendar.dateFormat.
 */
Mira.Calendar.prototype.callHandler = function () {
	if (this.onSelected) {
 ...
 */
Mira.Calendar.prototype.updateHistory = function () {
	var a, i, d, tmp, s, str = "", len = Mira.Calendar.prefs.hsize - 1;
	if (Mira.Calendar.prefs.history) {
		a = Mira.Calendar.prefs.history.split(/,/);
		i = 0;
		while (i < len && (tmp = a[i++])) {
	...
	Mira.Calendar.prefs.history = this.date.print("%Y/%m/%d/%H/%M") + str;
	Mira.Calendar.savePrefs();
};
 ...
 */
Mira.Calendar.prototype.callCloseHandler = function () {
	if (this.dateClicked) {
		this.updateHistory();
	}
	if (this.onClose) {
...
Mira.Calendar.prototype.destroy = function () {
	this.hide();		// this also removes keyboard events :-\
	Mira.Utils.destroy(this.element);
	Mira.Utils.destroy(this.WCH);
	Mira.Calendar._C = null;
 ...
 */
Mira.Calendar.prototype.reparent = function (new_parent) {
	var el = this.element;
 ...
 */
Mira.Calendar._checkCalendar = function(ev) {
	if (!window.calendar) {
		return false;
	}
	var el = Mira.is_ie ? Mira.Utils.getElement(ev) : Mira.Utils.getTargetElement(ev);
 ...
 */
Mira.Calendar.prototype.updateWCH = function(other_el) {
	Mira.Utils.setupWCH_el(this.WCH, this.element, other_el);
};
 ...
 */
Mira.Calendar.prototype.show = function () {
	var rows = this.table.getElementsByTagName("tr");
	for (var i = rows.length; i > 0;) {
		var row = rows[--i];
		Mira.Utils.removeClass(row, "rowhilite");
		var cells = row.getElementsByTagName("td");
		for (var j = cells.length; j > 0;) {
			var cell = cells[--j];
			Mira.Utils.removeClass(cell, "hilite");
			Mira.Utils.removeClass(cell, "active");
		}
	}
	...
	if (this.isPopup) {
		...
		if (!this.noGrab) {
			Mira.Utils.addEvent(window.document, "keydown", Mira.Calendar._keyEvent);
			Mira.Utils.addEvent(window.document, "keypress", Mira.Calendar._keyEvent);
			Mira.Utils.addEvent(window.document, "mousedown", Mira.Calendar._checkCalendar);
		}
	}
	this.hideShowCovered();
};
 ...
 * mousedown.
 */
Mira.Calendar.prototype.hide = function () {
	if (this.isPopup) {
		Mira.Utils.removeEvent(window.document, "keydown", Mira.Calendar._keyEvent);
		Mira.Utils.removeEvent(window.document, "keypress", Mira.Calendar._keyEvent);
		Mira.Utils.removeEvent(window.document, "mousedown", Mira.Calendar._checkCalendar);
	}
	this.element.style.display = "none";
	Mira.Utils.hideWCH(this.WCH);
	this.hidden = true;
	this.hideShowCovered();
};
 ...
 */
Mira.Calendar.prototype.showAt = function (x, y) {
 ...
 */
Mira.Calendar.prototype.showAtElement = function (el, opts) {
	var self = this;
	var p = Mira.Utils.getAbsolutePos(el);
 ...
 */
Mira.Calendar.prototype.setDateFormat = function (str) {
	this.dateFormat = str;
};

/** Customizes the tooltip date format.  See
 * Mira.Calendar.prototype.setDateFormat() for a description of the \em str
 * format.
 *
 * @param str [string] the "tooltip" date format
 */
Mira.Calendar.prototype.setTtDateFormat = function (str) {
 ...
 */
Mira.Calendar.prototype.parseDate = function (str, fmt) {
	// Konqueror
	if (!str)
 ...
 */
Mira.Calendar.prototype.hideShowCovered = function () {
	if (!Mira.is_ie5)
		return;
	var self = this;
	...
	for (var k = tags.length; k > 0; ) {
		...
		for (var i = ar.length; i > 0;) {
			...
			p = Mira.Utils.getAbsolutePos(cc);
			var CX1 = p.x;
 ...
 */
Mira.Calendar.prototype._displayWeekdays = function () {
	...
	for (var i = 0; i < 7; ++i) {
		...
		if (i) {
			if (Mira.Calendar.i18n("DAY_FIRST") != null) {
				cell.ttip = Mira.Calendar.i18n("DAY_FIRST").replace("%s", Mira.Calendar.i18n(realday, "dn"));
			}
			cell.navtype = 100;
			cell.calendar = this;
			cell.fdow = realday;
		...
		if ((weekend != null) && (weekend.indexOf(realday.toString()) != -1)) {
						Mira.Utils.addClass(cell, "weekend");
		}
		cell.innerHTML = Mira.Calendar.i18n((i + fdow) % 7, "sdn");
		cell = cell.nextSibling;
	}
};

/** \internal Hides all combo boxes that might be displayed. */
Mira.Calendar.prototype._hideCombos = function () {
...
Mira.Calendar.prototype._dragStart = function (ev) {
	ev || (ev = window.event);
	if (this.dragging) {
		return;
	}
	...
	Mira.Utils.addEvent(window.document, "mousemove", Mira.Calendar.calDragIt);
	Mira.Utils.addEvent(window.document, "mouseover", Mira.Calendar.calDragIt);
	Mira.Utils.addEvent(window.document, "mouseup", Mira.Calendar.calDragEnd);
};
...
Date.prototype.print = function (str) {
	...
	s["%a"] = Mira.Calendar.i18n(w, "sdn"); // abbreviated weekday name [FIXME: I18N]
	s["%A"] = Mira.Calendar.i18n(w, "dn"); // full weekday name
	s["%b"] = Mira.Calendar.i18n(m, "smn"); // abbreviated month name [FIXME: I18N]
	s["%B"] = Mira.Calendar.i18n(m, "mn"); // full month name
	...
	var re = /%./g;
	if (!Mira.is_ie5 && !Mira.is_khtml && !Mira.is_mac_ie)
...
Date.parseDate = function (str, fmt) {
	...
	for (i = 0; i < a.length; ++i) {
		...
		switch (b[i]) {
		    ...
		    case "%B":
			for (j = 0; j < 12; ++j)
				if (Mira.Calendar.i18n(j, "mn").substr(0, a[i].length).toLowerCase() == a[i].toLowerCase()) {
					m = j;
					break;
				}
			break;
