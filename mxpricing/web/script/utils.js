...
/// define the global Mira namespace
var Mira = {};
...
Mira.Utils.getAbsolutePos = function(el) {
	var SL = 0, ST = 0;
...
Mira.Utils.fixBoxPosition = function(box) {
	if (box.x < 0)
		box.x = 0;
	if (box.y < 0)
		box.y = 0;
	var cp = Mira.Utils.createElement("div");
	var s = cp.style;
	s.position = "absolute";
	s.right = s.bottom = s.width = s.height = "0px";
	window.document.body.appendChild(cp);
	var br = Mira.Utils.getAbsolutePos(cp);
...
///         if (!Mira.Utils.isRelated(this, event)) {
...
Mira.Utils.isRelated = function (el, evt) {
...
Mira.Utils.removeClass = function(el, className) {
	if (!(el && el.className)) {
		return;
	}
...
Mira.Utils.addClass = function(el, className) {
	Mira.Utils.removeClass(el, className);
...
Mira.Utils.getElement = function(ev) {
	if (Mira.is_ie) {
...
Mira.Utils.getTargetElement = function(ev) {
	if (Mira.is_ie) {
		return window.event.srcElement;
	} else {
		return ev.target;
...
/// @param ev [Event] the event object
/// @return false
Mira.Utils.stopEvent = function(ev) {
	ev || (ev = window.event);
	if (Mira.is_ie) {
...
Mira.Utils.addEvent = function(el, evname, func) {
...
/// Removes an event handler added with Mira.Utils.removeEvent().  The
/// prototype scheme is the same.
Mira.Utils.removeEvent = function(el, evname, func) {
...
Mira.Utils.createElement = function(type, parent) {
	var el = null;
...
Mira.Utils.writeCookie = function(name, value, domain, path, exp_days) {
	value = escape(value);
 ...
 */
Mira.Utils.getCookie = function(name) {
 ...
 */
Mira.Utils.makePref = function(obj) {
...
/** 
 * The reverse of Mira.Utils.makePref(), this function unserializes the
 ...
 */
Mira.Utils.loadPref = function(txt) {
	var obj = null;
	try {
		eval("obj={" + txt + "}");
	} catch(e) {}
 ...
 */
Mira.Utils.mergeObjects = function(dest, src) {
	for (var i in src)
		dest[i] = src[i];
};
...
Mira.Utils.__wch_id = 0;	/**< [number, static] used to create ID-s for the WCH objects */
 ...
 */
Mira.Utils.createWCH = function() {
	var f = null;
	if (Mira.is_ie && !Mira.is_ie5) {
		var filter = 'filter:progid:DXImageTransform.Microsoft.alpha(style=0,opacity=0);';
		var id = "WCH" + (++Mira.Utils.__wch_id);
 ...
 */
Mira.Utils.setupWCH_el = function(f, el, el2) {
	if (f) {
		...
		if (el2) {
			var p2 = Mira.Utils.getAbsolutePos(el2),
				XX1 = p2.x,
				YY1 = p2.y,
		...
		Mira.Utils.setupWCH(f, X1, Y1, X2-X1, Y2-Y1);
	}
};
 ...
 */
Mira.Utils.setupWCH = function(f, x, y, w, h) {
	if (f) {
		var s = f.style;
 ...
 */
Mira.Utils.destroy = function(el) {
	if (el && el.parentNode)
 ...
 */
Mira.Utils.newCenteredWindow = function(url, windowName, width, height, scrollbars){
	var leftPosition = 0;
	var topPosition = 0;
	if (screen.width)
 ...
 */
Mira.Utils.selectOption = function(sel, val, call_default) {
 ...
 */
Mira.Utils.getNextSibling = function(el, tag) {
	el = el.nextSibling;
	if (!tag)
		return el;
	tag = tag.toLowerCase();
...
/** 
 * Similar to Mira.Utils.getNextSibling(), this function will return the
 ...
 * found.
 */
Mira.Utils.getFirstChild = function(el, tag) {
	el = el.firstChild;
	if (!tag)
		return el;
	tag = tag.toLowerCase();
	if (el.nodeType == 1 && el.tagName.toLowerCase() == tag)
		return el;
	return Mira.Utils.getNextSibling(el, tag);
};

Mira.Utils._ids = {};	/**< [number, static] maintains a list of generated IDs */
 ...
 */
Mira.Utils.generateID = function(code, id) {
...
Mira.Utils.addTooltip = function(target, tooltip) {
return new Mira.Tooltip(target, tooltip);
};
...
Mira.is_opera = /opera/i.test(navigator.userAgent);

/// detect a special case of "web browser"
Mira.is_ie = ( /msie/i.test(navigator.userAgent) && !Mira.is_opera );

/// detect IE5.0/Win
Mira.is_ie5 = ( Mira.is_ie && /msie 5\.0/i.test(navigator.userAgent) );

/// detect IE for Macintosh
Mira.is_mac_ie = ( /msie.*mac/i.test(navigator.userAgent) && !Mira.is_opera );

/// detect KHTML-based browsers
Mira.is_khtml = /Konqueror|Safari|KHTML/i.test(navigator.userAgent);

/// detect Konqueror
Mira.is_konqueror = /Konqueror/i.test(navigator.userAgent);

/// detect Gecko
Mira.is_gecko = /Gecko/i.test(navigator.userAgent);
