 ...
 * 
 * Class Name IDRProfileDetailsServlet.java
 * 
...
import com.savant.pricing.dao.IDRDAO;
 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class IDRProfileDetailsServlet extends HttpServlet{
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        try 
        { 
            String essid = request.getParameter("ESIID");
            ...
            if(essid!=null||!essid.equalsIgnoreCase("")||!essid.equals(null))
            {
                IDRDAO objIDRDAO = new IDRDAO();
                HashMap hmIDRDetails = objIDRDAO.getIDRProfileDetails(essid);
                HashMap hmIDRWeekDayDetails = (HashMap) hmIDRDetails.get(new Integer(2));
                HashMap hmIDRWeekEndDetails = (HashMap) hmIDRDetails.get(new Integer(3));
                ProfileChart objProfileChart = new ProfileChart();
                HashMap filename =  objProfileChart.getchart(request.getSession(),hmIDRDetails,response.getWriter());
                ...
                keySet = hmIDRWeekEndDetails.keySet();
                iteMonth = keySet.iterator();
                while(iteMonth.hasNext())
                {
                    int monthCount = ((Integer)iteMonth.next()).intValue();
                    List lsthourDetails = (List)hmIDRWeekEndDetails.get(new Integer(monthCount));
                    int hourCount = 1;
                    if(lsthourDetails!=null)
                    while(hourCount<=lsthourDetails.size())
                    {
...
/*
*$Log: IDRProfileDetailsServlet.java,v $
